...
export async function getProductConfig(): Promise<ActionResult<ProductConfigResponse & { areas?: string[] }>> {
  try {
    ...
    return errorResponse('Lỗi truy vấn danh mục sản phẩm: ' + (error as Error).message);
  }
}
